...
# セクション: 代理店別LTV分析
# ============================
st.markdown('<div class="section-card"><div class="section-title">代理店別LTV分析</div>', unsafe_allow_html=True)

# ordersのchannel列を使って全期間のチャネル別LTVを集計
_ltv_with_channel = df_ltv.copy()
_ltv_with_channel["channel"] = _ltv_with_channel["用户邮箱"].map(
    df_orders.groupby("用户邮箱")["channel"].first()
).fillna("サポートサイト")

_channel_ltv = _ltv_with_channel.groupby(["用户邮箱", "channel"]).agg(
    ltv=("金额", "sum"),
    order_count=("金额", "count"),
    first_order=("order_date", "min"),
    last_order=("order_date", "max"),
).reset_index()
_channel_ltv["is_repeater"] = _channel_ltv["order_count"] >= 2
_channel_ltv["tenure_months"] = (_channel_ltv["last_order"] - _channel_ltv["first_order"]).dt.days / 30.44

_channel_summary = _channel_ltv.groupby("channel").agg(
    ユーザー数=("用户邮箱", "nunique"),
    平均LTV=("ltv", "mean"),
    平均注文回数=("order_count", "mean"),
    リピート率=("is_repeater", "mean"),
    平均継続月数=("tenure_months", lambda x: x[_channel_ltv.loc[x.index, "is_repeater"]].mean() if _channel_ltv.loc[x.index, "is_repeater"].any() else 0),
).reset_index()
_channel_summary.columns = ["チャネル", "ユーザー数", "平均LTV", "平均注文回数", "リピート率", "平均継続月数"]
_channel_summary["平均LTV"] = _channel_summary["平均LTV"].round(1)
_channel_summary["平均注文回数"] = _channel_summary["平均注文回数"].round(1)
_channel_summary["リピート率"] = (_channel_summary["リピート率"] * 100).round(1)
_channel_summary["平均継続月数"] = _channel_summary["平均継続月数"].round(1)
_channel_summary = _channel_summary.sort_values("ユーザー数", ascending=False)

col1, col2 = st.columns([1, 2])
with col1:
    styled_table(_channel_summary, t)
with col2:
    fig_ch_ltv = px.bar(_channel_summary, x="チャネル", y="平均LTV",
                        title="チャネル別 平均LTV（USD）", text="平均LTV",
                        color_discrete_sequence=[t["accent"]])
    fig_ch_ltv.update_traces(texttemplate="$%{text:,.0f}")
    fig_ch_ltv.update_layout(**PLOT_LAYOUT)
    st.plotly_chart(fig_ch_ltv, use_container_width=True, theme=None)

# 自社 vs 代理店の比較サマリー
_self_ltv = _channel_ltv[_channel_ltv["channel"].isin(["サポートサイト", "公式サイト"])]
_agent_ltv = _channel_ltv[~_channel_ltv["channel"].isin(["サポートサイト", "公式サイト"])]

if len(_agent_ltv) > 0:
    st.markdown('<div class="sub-title">自社 vs 代理店 比較</div>', unsafe_allow_html=True)
    cols = st.columns(4)
    self_avg = _self_ltv["ltv"].mean() if len(_self_ltv) > 0 else 0
    agent_avg = _agent_ltv["ltv"].mean() if len(_agent_ltv) > 0 else 0
    self_repeat = (_self_ltv["is_repeater"].mean() * 100) if len(_self_ltv) > 0 else 0
    agent_repeat = (_agent_ltv["is_repeater"].mean() * 100) if len(_agent_ltv) > 0 else 0
    cols[0].markdown(kpi_card("自社 平均LTV", f"${self_avg:,.0f}", "blue",
        tooltip='<strong>自社チャネル平均LTV</strong>サポートサイト＋公式サイト経由ユーザーの累計課金額平均。'), unsafe_allow_html=True)
    cols[1].markdown(kpi_card("代理店 平均LTV", f"${agent_avg:,.0f}", "green",
        tooltip='<strong>代理店チャネル平均LTV</strong>代理店経由ユーザーの累計課金額平均。'), unsafe_allow_html=True)
    cols[2].markdown(kpi_card("自社 リピート率", f"{self_repeat:.1f}%", "blue",
        tooltip='<strong>自社リピート率</strong>サポートサイト＋公式サイト経由で2回以上注文したユーザーの割合。'), unsafe_allow_html=True)
    cols[3].markdown(kpi_card("代理店 リピート率", f"{agent_repeat:.1f}%", "green",
        tooltip='<strong>代理店リピート率</strong>代理店経由で2回以上注文したユーザーの割合。'), unsafe_allow_html=True)
